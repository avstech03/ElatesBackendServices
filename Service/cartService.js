...
class CartService {
    ...
    calculateCart(cart, membershipObtained) {
        ...
        return Promise.join(getCouponP, getSeedDataP)
            .spread((coupon, seedData) => {
                let amount = 0;
                // Calculate product prices
                if (cart.coupon && !coupon) return Promise.reject("INVALID_COUPON");
                if (cart.products && cart.products.length) {
                    _.forEach(cart.products, product => {
                        if (membershipObtained || (cart.membership && cart.membership.planName && cart.membership.price))
                            amount += product.specialPrice * product.count;
                        else
                            amount += product.sellingPrice * product.count;
                    });
                    if (cart.deliveryType === 'multiple') {
                        amount = calculateMultipleOrderPrice(amount, cart.from, cart.to, cart.frequency);
                        cart.deliveryCharges = 0;
                    } 
                    else if (!membershipObtained && (!cart.membership || !cart.membership.planName || !cart.membership.price)) {
                        amount += seedData.deliveryCharges;
                        cart.deliveryCharges = seedData.deliveryCharges;
                    } 
                    if(cart.membership && cart.membership.planName && cart.membership.price) {
                        amount += parseInt(cart.membership.price);
                        cart.deliveryCharges = 0;
                    }

                    if (coupon && coupon.restrictions && coupon.restrictions.priceMoreThan <= amount) {
                        let discount = parseInt((amount * coupon.restrictions.discountPercent) / 100);
                        if (discount > coupon.restrictions.maximumDiscount) discount = coupon.restrictions.maximumDiscount;
                        amount -= discount;
    ...
    addProductToCart(userId, cartId, product) {
        ...
        return Promise.join(getCartP, getUserP, seedDataP)
            .spread((cart, user, seedData) => {
                if (cart && cart.products) {
                    let productAlreadyExists = false;
                    _.forEach(cart.products, _product => {
                        if (_product.productId === product.productId && _product.quantity === product.quantity) {
                    ...
                    return this.calculateCart(cart, user.membershipObtained)
                        .then(cart => {
                            return this.model.updateCartById(cartId, cart)
                                .then(res => {
                                    return this.model.getCartByUserId(userId);
                                });
                        });
    ...
    applyCoupon(userId, cartId, couponCode) {
        ...
        return Promise.join(getCartP, getUserP)
            .spread((cart, user) => {
    ...
    saveSingleOrder(userId, cartId, body) {
        ...
        const seedDataModel = ModelFactory.seeddata();
        const getSeedDataP = seedDataModel.getSeedData();
        return Promise.join(getCartP, getUserP, getSeedDataP)
            .spread((cart, user, seedData) => {
    ...
    createOrderForRazorPay(userId, body) {
        ...
        .then(cart => {
            if(cart) {
                ...
                return razonpayInstance.orders.create({
                    amount: amount * 100,
                ...
                .then(razorPayOrder => {
                    ...
                    return paymentStatusModel.createPaymentStatus(obj)
                    .then(() => {
                        return {
    ...
    verifyRazorPayPaymentSuccess(body) {
        ...
        const paymentStatusModel = ModelFactory.paymentstatus();
        return paymentStatusModel.updatePaymentStatus({paymentOrderId}, {
            status: "success"
        });
    }
